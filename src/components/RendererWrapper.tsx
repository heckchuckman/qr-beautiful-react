...
export interface StyledFunctionComponent<P = {}>
...
export const RendererWrapper = <T extends RendererProps>(renderer: SFC<T>) => {
	const Renderer: SFC<T> = (props: T) => {
		...
		newProps.value = newProps.value || "https://qrlove.link"
		newProps.level =
			newProps.title || newProps.icon ? "H" : newProps.level || "M"
		newProps.qrcode =
			newProps.qrcode ||
			encodeData({
				text: newProps.value,
		...
		return React.createElement(renderer, newProps)
	}
	return Renderer
}
...
export function drawIcon({
	...
	if (title) {
		const svgWidth =
			styles.svg && styles.svg.width
				? styles.svg.width.replace("px", "")
				: 300
		const titleFontSize =
			(Number(nCount + (nCount / 5) * 2) *
				(titleSize || fontSize || 12)) /
			svgWidth
